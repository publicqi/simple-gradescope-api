...
class GSAccount():
    ...
    def add_class(self, cid, name, shortname, year, instructor=False):
        if instructor:
            self.instructor_courses[cid] = GSCourse(
    ...
    def create_course(self, name, shortname, description, term, year, school, entry_code_enabled=False):
        ...
        # TODO Fix this on bad params.
        school_id = schools.find('option', text=school).get('value')
        course_data = {
            "utf8": "✓",
        ...
        self.add_class(cid, name, shortname, term +
                       " " + year, instructor=True)
        return cid
